...
import { IPaymentInfo, InAppSubscription, PaymentInfo, isSubscriptionId } from "@/models/PaymentInfo";
import { syncDataToWebAfterLoginAPI } from "@/services/importAppData";
import { createAsyncThunk } from "@reduxjs/toolkit";
import AppState from "../appState";

/** lấy dữ liệu payment (web), inAppSubscription (mobile) và update vào redux, gọi mỗi khi vào trang */
const getUserDeviceLogin = createAsyncThunk("getUserDeviceLogin", async (appInfo: IAppInfo, { getState }) => {
    const state = getState() as AppState;
    const userInfo = state.userReducer.userInfo;
    const paymenyInfos = state.userReducer.paymentInfos;
    if (!!userInfo) {
        try {
            ...
            let userDeviceLogin = syncData.UserDeviceLogins.filter((u) => u);
            ...
            let { paymentInfo, paymentInfos } = handleUserDevicesLogin(userDeviceLogin, appInfo, paymenyInfos);
 ...
 */
const handleUserDevicesLogin = (userDeviceLogins: any, appInfo: IAppInfo, paymentInfos: IPaymentInfo[]) => {
    userDeviceLogins = userDeviceLogins.filter((t) => t != null && t != undefined);
    try {
        ...
        for (const userDeviceLogin of userDeviceLogins) {
            ...
            let paymentInfoExist = paymentInfos.find((_paymentInfo) => _paymentInfo.appId == userDeviceLogin.appId);
            if (!paymentInfoExist) {
                ...
                paymentInfoExist = new PaymentInfo({
                    ...
                    paymentStatus: userDeviceLogin.inAppPurchared == 1 ? Config.PAYMENT_SUCCESS : Config.PAYMENT_INIT,
                    appShortName: userDeviceLogin.appShortName,
                    expiredDate: userDeviceLogin?.expiredDate ?? -1,
                    purchared: userDeviceLogin.inAppPurchared == 1 ? Config.PURCHARED : Config.PURCHAR,
                });
            } else {
                ...
                if (userDeviceLogin.inAppPurchared == 1) {
                    paymentInfoExist.purchared = userDeviceLogin.inAppPurchared;
                }
            }
            ...
            if (userDeviceLogin.buyPro == Config.PURCHARED || userDeviceLogin.inAppPurchared == Config.PURCHARED) {
                paymentInfoExist.buyPro = Config.PURCHARED;
                paymentInfoExist.purchared = Config.PURCHARED;
            }

            if (isSubscriptionId(paymentInfoExist.orderId)) {
                localStorage.setItem("isSubcriptionPro", Config.PAYMENT_PAY_SUBSCRIPTION);
            }
