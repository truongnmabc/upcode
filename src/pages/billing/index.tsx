...
import { SUBSCRIPTION, getConfigPro, getConfigProV2 } from "../../config/config_paypal";
...
const BillingPage = ({ appInfo }: { appInfo: IAppInfo }) => {
    ...
    const inAppSubscriptions = userReducer.inAppPruchasedInfo;
    const [loading, setLoading] = useState(false);
    const [listBill, setListBill] = useState<{ date: string; period: string; plan: string; amount: string; status: string }[]>(
        []
    );
    ...
    useEffect(() => {
        ...
        if (paymentInfo?.orderId && isSubscriptionId(paymentInfo?.orderId) && type === SUBSCRIPTION) {
            getBilling();
        }
    }, [paymentInfo?.orderId]);
    ...
    const handleClickGetPro = (index: number) => {
        ...
        let currentPrice = 0;
        let stateValue = dataConfig.config[dataConfig.config.length - 1].bottomContent.stateValue;
        currentPrice = price;
        setValueButton({
            price: currentPrice + "",
            value: currentPrice,
    ...
    if (listBill.length > 0) {
        next_billing_time = listBill[0].period.slice(15, listBill[0].period.length);
    }
    return (
        <>
            ...
            <StoreProvider appInfo={appInfo} webData={{ appId: appInfo.appId, type: "", getUserData: true }} />
                    ...
                    <div className="pro-background">
                        {inAppSubscriptions.length > 0 && (
                            <div className="alert-in-app-purchased">
                                <Alert />
                                You made your purchase on our mobile app. If you want to change or cancel your current plan,
                    ...
                    <MyContainer className="billing-history">
                        {!!userInfo && !!next_billing_time && (
                            <MyContainer className="next-billing-date">
                                <div className="line" />
                                Your next billing date is {next_billing_time} <div className="line" />
                            </MyContainer>
                        )}
                        <span>Billing History {loading && <V4CircleProgress size={16} thickness={2} />}</span>
                            ...
                            <table>
                                <thead>
                                    <tr>
                                        ...
                                        <th>Plan Name</th>
                                        <th>{isDesktop ? "Amount (USD)" : "Amount"}</th>
                                        <th>Status</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {listBill.map((b, i) => (
                                        <tr key={i}>
                                            <td>{isDesktop ? b.date : <Period />}</td>
                                            <td>
                                                {isDesktop
                                                    ? b.period
                                                    : b.period.split(" to ").map((p, _i) => <div key={_i}>{p}</div>)}
                                            </td>
                                            <td>{b.plan}</td>
                                            <td>{b.amount}</td>
                                            <td>{b.status}</td>
                                        ...
                                        <tbody>
                                            {inAppSubscriptions.map((b, i) => {
                                                return b.in_app.map((_, __) => {
                                                    let period =
                                                        _formatTime(_.purchase_date) + " to " + _formatTime(_.expires_date);
                                                    return (
                                                        <tr key={"" + i + __}>
                                                            <td>{isDesktop ? _formatTime(_.purchase_date) : <Period />}</td>
                                                            <td>
                                                                {isDesktop
                                                                    ? period
                                                                    : period
                                                                          .split(" to ")
                                                                          .map((p, _i) => <div key={_i + "_"}>{p}</div>)}
                                                            </td>
                                                            <td>{b.os}</td>
                                                            <td>{_.in_app_ownership_type}</td>
                                                        </tr>
                                                    );
                                                });
                                            })}
    ...
    return (
        <svg width="16" height="41" viewBox="0 0 16 41" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
                ...
                fillOpacity="0.08"
            />
            <circle cx="8" cy="8" r="5.5" fill="white" stroke="#45866F" strokeWidth="5" />
            <circle cx="8" cy="37" r="4" fill="white" />
            <circle cx="8" cy="37" r="2.5" stroke="#45866F" strokeOpacity="0.52" strokeWidth="3" />
        </svg>
    );
};
