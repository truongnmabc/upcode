...
const Handbook = ({ appInfo, _state }: { appInfo: IAppInfo; _state: string }) => {
    return (
        <div className="v4-container-component pb-12 sm:pb-[120px] v4-container-maxWidth">
            <h3 className="sm:pt-6 mt-4 text-lg px-6 sm:px-0 text-center sm:text-start font-medium sm:text-2xl sm:font-semibold ">
                ...
                <span className=" uppercase">{appInfo?.appShortName}</span> handbook
            </h3>

            <div
                className="sm:mt-6 mt-4 h-fit flex flex-col-reverse sm:flex-row rounded-2xl"
                style={{
                    background: "linear-gradient(90deg, #D8DEFF 0%, #EED8FF 100%)",
                }}
            >
                <div className="flex-1 sm:p-6 p-4">
                    <p className="text-sm text-[#212121CC] sm:text-[#212121] sm:text-xl font-normal">
                        ...
                        is packed with all <span className="capitalize">{_state}</span>{" "}
                        <span className=" uppercase">{appInfo?.appShortName}</span> rules, regulations, and helpful tips, taking
                ...
                <Image src={"/images/cdl_v2/home/Rectangle.png"} width={300} height={228} className=" rounded-lg" />
            </div>
        </div>
    );
};
