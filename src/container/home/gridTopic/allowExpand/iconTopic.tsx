...
export const IconSubTopic = ({
  ...
  return (
    <div
      className={ctx("p-1 border-solid  w-10 h-10 rounded-full", {
        "heartbeat anim-boundary": activeAnim,
        "bg-white": lock,
        "bg-[#f5f5f5]": !lock,
      })}
      style={{
        border: "0.5px ",
        background: "linear-gradient(134.17deg, #FFFFFF 4.98%, #FFFFFF 94.88%)",
        borderImageSource:
      ...
      <div
        className={ctx(
          "rounded-full w-full h-full  p-2  flex items-center justify-center",
          {
            ...
            "bg-[#f5f5f5]": lock,
            "bg-[#15CB9F]":
              isFinishThisLevel || (!lock && currentLevelScore >= 50),
            "bg-[#EE6769] text-white": !lock && currentLevelScore < 50,
          }
        )}
      >
        {lock ? (
