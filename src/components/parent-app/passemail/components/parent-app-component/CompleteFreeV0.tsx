...
const CompleteFreeV0 = () => {
    ...
    const [contentSelected, setContentSelected] = useState(listContent[0]?.id ?? 0);
    let mapData: { [data: string]: DataType } = {};
    for (let content of listContent) {
        if (content.id) {
            if (!mapData[content.id]) {
                mapData[content.id] = { id: 0, description: "", image: "", title: "" };
            }
            ...
            <div className="completely-free-V0">
                <MyContainer className="completely-free">
                    <div className="completely-container">
                        <img className="image" src={mapData[contentSelected].image} alt="image" />
                        <div className="completely">
                            {listContent?.map((data, index) => {
                                return (
                                    <div
                                        id={"completely-item-" + data.id}
                                        className={"completely-item " + (contentSelected === data.id ? "active" : "")}
