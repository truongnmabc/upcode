...
props(DexiePromise.prototype, {
    ...
    _then: function (onFulfilled, onRejected) {
        ...
        return this.then(
            ...
            (err) => {
                return DexiePromise.resolve(onFinally()).then(() =>
                    PromiseReject(err)
                );
            }
        );
...
class Table {
    ...
    where(indexOrCrit) {
        ...
        const compoundIndex = this.schema.indexes
            ...
            .filter((ix) => {
                if (
                    ix.compound &&
                    keyPaths.every(
                        (keyPath) => ix.keyPath.indexOf(keyPath) >= 0
                    )
                ) {
        ...
        if (compoundIndex && this.db._maxKey !== maxString) {
            const keyPathsInValidOrder = compoundIndex.keyPath.slice(
                0,
                keyPaths.length
            );
            return this.where(keyPathsInValidOrder).equals(
                keyPathsInValidOrder.map((kp) => indexOrCrit[kp])
            );
        }
                    ...
                    prevIndex || !index
                        ? combine(
                              ...
                              index && index.multi
                                  ...
                                  : (x) =>
                                        equals(value, getByKeyPath(x, keyPath))
                          )
                        : prevFilterFn,
                ];
            },
    ...
    bulkUpdate(keysAndChanges) {
        ...
        return this._trans("readwrite", (trans) => {
            return coreTable
                .getMany({ trans, keys, cache: "clone" })
                .then((objs) => {
                    const resultKeys = [];
                    const resultObjs = [];
...
class PropModification {
    execute(value) {
        ...
        if (
            ...
            return (
                this.replacePrefix[1] + value.substring(prefixToReplace.length)
            );
        }
        return value;
    }
        ...
        return this._write((trans) => {
            ...
            if (modifyChunkSize) {
                if (typeof modifyChunkSize == "object") {
                    limit =
                        modifyChunkSize[coreTable.name] ||
                        modifyChunkSize["*"] ||
                        200;
                } else {
                        ...
                        return coreTable
                            ...
                            .then((values) => {
                                ...
                                for (let i = 0; i < count; ++i) {
                                    ...
                                    if (
                                        modifyer.call(ctx, ctx.value, ctx) !==
                                        false
                                    ) {
                                        if (ctx.value == null) {
...
function emptyCollection(whereClause) {
    return new whereClause.Collection(whereClause, () => rangeEqual("")).limit(
        0
    );
}
        ...
        if (this._locked()) {
            return new DexiePromise((resolve, reject) => {
                this._blockedFuncs.push([
                    () => {
                        this._promise(mode, fn, bWriteLock).then(
                            resolve,
                            reject
                        );
                    },
    ...
    waitFor(promiseLike) {
        ...
        return new DexiePromise((resolve, reject) => {
            promise
                .then(
                    (res) =>
                        root._waitingQueue.push(wrap(resolve.bind(null, res))),
                    (err) =>
                        root._waitingQueue.push(wrap(reject.bind(null, err)))
                )
                .finally(() => {
        ...
        function mutate({ trans, type, keys, values, range }) {
            return new Promise((resolve, reject) => {
                ...
                const done = (event) => {
                    ...
                    reqs.forEach(
                        (req, i) =>
                            req.error != null && (failures[i] = req.error)
                    );
                    resolve({
                        numFailures,
                        failures,
...
function patchCurrentVersion(db, idbUpgradeTrans) {
    ...
    for (const tableChange of diff.change) {
        if (tableChange.change.length || tableChange.recreate) {
            console.warn(
                `Unable to patch indexes of table ${tableChange.name} because it has changes on the type of index or primary key.`
            );
            return;
        }
        ...
        tableChange.add.forEach((idx) => {
            if (debug)
                console.debug(
                    `Dexie upgrade patch: Creating missing index ${tableChange.name}.${idx.src}`
                );
            addIndex(store, idx);
        });
    }
        ...
        queue.push(() => {
            ...
            if (contentUpgrade && version._cfg.version > oldVersion) {
                ...
                const promiseFollowed = DexiePromise.follow(() => {
                    ...
                    if (returnValue) {
                        if (contentUpgradeIsAsync) {
                            var decrementor = decrementExpectedAwaits.bind(
                                null,
                                null
                            );
        ...
        queue.push((idbtrans) => {
            if (db.idbdb.objectStoreNames.contains("$meta")) {
                if (Math.ceil(db.idbdb.version / 10) === version._cfg.version) {
                    ...
                    db._storeNames = db._storeNames.filter(
                        (name) => name !== "$meta"
                    );
                } else {
                    idbtrans
    ...
    for (const [indexName, entries] of Object.entries(tblCache.queries.query)) {
        ...
        for (const entry of entries) {
            if (obsSetsOverlap(updatedParts, entry.obsSet)) {
                entry.subscribers.forEach((requery) =>
                    outQueriesToSignal.add(requery)
                );
                ...
                if (objectStoreNames.length > 0)
                    try {
                        ...
                        if (state.autoSchema)
                            ...
                            if (
                                !verifyInstalledSchema(db, tmpTrans) &&
                                !schemaPatchMode
                            ) {
                                console.warn(
            ...
            if (wasCreated) {
                ...
                db.tables.forEach((table) => {
                    table.schema.indexes.forEach((idx) => {
                        if (idx.name)
                            everything[
                                `idb://${db.name}/${table.name}/${idx.name}`
                            ] = new RangeSet(-Infinity, [[[]]]);
                    });
                    everything[`idb://${db.name}/${table.name}/`] = everything[
                        `idb://${db.name}/${table.name}/:dels`
                    ] = new RangeSet(-Infinity, [[[]]]);
                });
            ...
            const result = {
                ...
                openCursor(req) {
                    ...
                    return table
                        ...
                        .then(
                            (cursor) => cursor && createVirtualCursor(cursor)
                        );
                },
            };
            return result;
            ...
            const tableMiddleware = {
                ...
                mutate(req) {
                    ...
                    function addPutOrDelete(req) {
                        const dxTrans = PSD.trans;
                        const keys =
                            req.keys || getEffectiveKeys(primaryKey, req);
            ...
            transaction: (stores, mode, options) => {
                if (PSD.subscr && mode !== "readonly") {
                    throw new exceptions.ReadOnly(
                        `Readwrite transaction in liveQuery context. Querier source: ${PSD.querier}`
                    );
                }
            ...
            table: (tableName) => {
                ...
                const tableClone = {
                    ...
                    mutate: (req) => {
                        ...
                        const mutatedParts =
                            req.mutatedParts || (req.mutatedParts = {});
                            ...
                            if (
                                ...
                                if (indexesWithAutoIncPK) {
                                    indexesWithAutoIncPK.forEach((idx) => {
                                        const idxVals = req.values.map((v) =>
                                            idx.extractKey(v)
                                        );
                                        const pkPos = idx.keyPath.findIndex(
                                            (prop) =>
                                                prop === primaryKey.keyPath
                                        );
                                        for (
                                            let i = 0, len = res.results.length;
                                            i < len;
                                            ++i
                                        ) {
                ...
                const getRange = ({ query: { index, range } }) => [
                    index,
                    new RangeSet(
                        range.lower ?? core.MIN_KEY,
                        range.upper ?? core.MAX_KEY
                    ),
                ];
                    ...
                    tableClone[method] = function (req) {
                        ...
                        if (isLiveQuery) {
                            ...
                            if (
                                ...
                                getRangeSet(queriedIndex.name || "").add(
                                    queriedRanges
                                );
                            }
                ...
                const tableMW = {
                    ...
                    mutate(req) {
                        ...
                        if (
                            (req.type === "add" || req.type === "put") &&
                            (req.values.length >= 50 ||
                                getEffectiveKeys(primKey, req).some(
                                    (key) => key == null
                                ))
                        ) {
                            promise.then((res) => {
                    ...
                    query(req) {
                        ...
                        const freezeResults =
                            PSD.trans?.db._options.cache === "immutable";
                        ...
                        if (cacheEntry && exactMatch) {
                            ...
                            if (container) {
                                ...
                                tblCache.queries.query[
                                    req.query.index.name || ""
                                ] = container;
                            }
                        }
                        subscribeToCacheEntry(
                            cacheEntry,
                            container,
                            requery,
                            signal
    ...
    constructor(name, options) {
        ...
        this._fireOnBlocked = (ev) => {
            ...
            connections
                .filter(
                    (c) =>
                        c.name === this.name && c !== this && !c._state.vcFired
                )
    ...
    _transaction(mode, tables, scopeFunc) {
        ...
        try {
            storeNames = tables.map((table) => {
                var storeName =
                    table instanceof this.Table ? table.name : table;
            ...
            if (parentTransaction) {
                ...
                if (parentTransaction) {
                    storeNames.forEach((storeName) => {
                        if (
                            parentTransaction &&
                            parentTransaction.storeNames.indexOf(storeName) ===
                                -1
                        ) {
                            if (onlyIfCompatible) {
                                parentTransaction = null;
